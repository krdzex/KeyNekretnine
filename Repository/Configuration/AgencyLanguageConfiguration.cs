...
public class AgencyLanguageConfiguration : IEntityTypeConfiguration<AgencyLanguage>
{
    public void Configure(EntityTypeBuilder<AgencyLanguage> builder)
    {
        ...
        builder.HasOne(x => x.Agency).WithMany(x => x.AgencyLanguage).HasForeignKey(x => x.AgencyId);
        builder.HasOne(x => x.Language).WithMany(x => x.AgencyLanguage).HasForeignKey(x => x.LanguageId);
    }
}
